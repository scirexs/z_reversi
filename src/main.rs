...
use dialog::DialogFrame;
use game_board::{ConfigData, PlayData, SetData, StoneType, PlayTurn, DifficultyLevel};
use asset::{DialogType, *};
use npc_pattern::*;
...
mod npc_pattern {
    use super::get_random_bool;
    use crate::game_board::{DifficultyLevel, PlayData, Address};

    pub fn get_move_easy(play_data: &PlayData) -> Address {
        let mut move_addr = Address::new((0, 0));
    ...
    pub fn get_move_normal(play_data: &PlayData) -> Address {
        if play_data.availables.len() == 1 { return *play_data.availables.first().unwrap_or(&Address::new((0, 0))); }
        let mut move_addr = Address::new((0, 0));
...
mod game_board {
    ...
    pub struct PlayData {
        pub first_turn: PlayTurn,
        pub level: DifficultyLevel,
        disp_avail: bool,
        pub turn: StoneType,
        board: [[StoneType; 8]; 8],
        pub availables: Vec<Address>,
    ...
    impl PlayData {
        ...
        pub fn sim_game(&mut self, addr: &Address) -> bool {
            if !self.availables.contains(addr) { return false; }
            self.move_stone(addr);
            self.pass_turn();
            true
        ...
        pub fn get_stone_addrs(&self, stone: StoneType) -> Vec<Address> {
            Self::get_board_addrs().into_iter().filter(|x| self.get_stone(x) == stone).collect()
        }
        pub fn get_board_status(&self) -> String {
        ...
        pub fn get_stable_addrs(&self, stone: StoneType) -> HashSet<Address> {
    ...
    impl Address {
        ...
        pub fn new(rc: (usize, usize)) -> Self { Self(rc.0, rc.1) }
        pub fn get(&self) -> (usize, usize) { (self.0, self.1) }
...
mod dialog {
    ...
    use anyhow::{bail, Result};
    use super::InputFunc;
    use crate::game_board::DataEnum;
    use crate::asset::DialogType;

    #[derive(Clone)]
    pub struct StatusShift {
        pub status: DataEnum,
        pub next: DialogType,
    }
    impl StatusShift {
    ...
    impl DialogFrame {
        pub fn new<T: Into<String>>(info: T, action_preset: Vec<(T, T, T, DataEnum, DialogType)>, input_func: InputFunc) -> Self {
        ...
        pub fn begin(&self) -> Result<StatusShift> {
...
mod asset {
    use super::InputFunc;
    use crate::game_board::{DataEnum, PlayTurn, DifficultyLevel, PlayData, Address};
    ...
    impl DialogType {
        pub fn from_turn(turn: &PlayTurn) -> Self {
            match turn {
    ...
    pub fn get_config_turn() -> DialogFrame {
        DialogFrame::new(
            "Choose your turn.",
            vec![
    ...
    pub fn get_config_level() -> DialogFrame {
        DialogFrame::new(
            "Choose NPC level.",
            vec![
    ...
    pub fn get_config_avail() -> DialogFrame {
        DialogFrame::new(
    ...
    pub fn get_game_env() -> DialogFrame {
        DialogFrame::new(
            "",
            vec![
    ...
    pub fn get_game_player(play_data: &PlayData) -> DialogFrame {
        DialogFrame::new(
    ...
    pub fn get_game_npc(addr: Address) -> DialogFrame {
        DialogFrame::new(
            "",
            vec![
...
#[cfg(test)]
mod tests{
    // use crate::{dialog::*, game_board::*, asset::*, npc_pattern::*};
    // use super::*;
